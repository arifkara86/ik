...
public class EmployeeRepository : IEmployeeRepository
{
    ...
    public async Task<Employee?> GetByIdWithDepartmentAsync(Guid id, CancellationToken cancellationToken = default) {
    ...
    public async Task<IReadOnlyList<Employee>> GetWhereAsync(Expression<Func<Employee, bool>> predicate, CancellationToken cancellationToken = default) {
         return await _dbContext.Employees.Where(predicate).ToListAsync(cancellationToken);
    }
